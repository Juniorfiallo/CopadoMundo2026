(() => {
  ...
  let wasmWinner = null;
  const storedBracketZoom = Number(localStorage.getItem("copa2026-bracket-zoom-v4"));
  let bracketZoom = Number.isFinite(storedBracketZoom) && storedBracketZoom > 0 ? storedBracketZoom : (window.innerWidth <= 760 ? 2.35 : 1);
  ...
  function contenderCarouselScorers() {
    ...
    const wanted = ["ismael saibari", "johan manzambi", "jonathan david", "julian quinones", "matheus cunha"]
      .map(key => ordered.find(item => normalizeNameKey(item.name) === key))
      .filter(item => item && activeTeams.has(item.team) && (item.goals ?? 0) >= 3);
    if (wanted.length) return wanted.slice(0, 5);
    const topKeys = new Set(mainCarouselScorers().map(item => scorerKey(item.name, item.team)));
    return ordered
      .filter(item => activeTeams.has(item.team) && !topKeys.has(scorerKey(item.name, item.team)) && (item.goals ?? 0) >= 3)
      .slice(0, 5);
  }
  ...
  function applyBracketZoom(recenter = true) {
    ...
    localStorage.setItem("copa2026-bracket-zoom-v4", String(bracketZoom));
