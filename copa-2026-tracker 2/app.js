(() => {
  ...
  function refreshScorersFromGoalEvents() {
    ...
    const seededTallies = buildKnockoutGoalScorerMap(baseData.goalEvents || {});
  ...
  function loadSaved() {
    try {
      ...
      if (isLegacyScorerData) {
        ...
        if (Array.isArray(saved.scorers)) scorerSeed = normalizeScorers(saved.scorers);
